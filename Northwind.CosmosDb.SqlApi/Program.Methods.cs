        ...
        try
        {
                ...
                authKeyOrResourceToken: primaryKey))
            {
                Container container = client
                    .GetContainer(databaseId: "Northwind", containerId: "Products");
                ...
                while(resultsIterator.HasMoreResults)
                {
                    ...
                    foreach (ProductCosmos product in products)
                    {
                        WriteLine("id: {0}, productName: {1}, unitPrice: {2}", arg0: product.id,
                            arg1: product.productName, arg2: product.unitPrice);
                    }
                }
            }
        }
                ...
                authKeyOrResourceToken: primaryKey))
            {
                ...
                while(resultIterator.HasMoreResults)
                {
                    ...
                    foreach (ProductCosmos product in products)
                    {
                        ...
                        WriteLine("Status code: {0}, Request charge: {1} RUs",
                            arg0: response.StatusCode, arg1: response.RequestCharge);
