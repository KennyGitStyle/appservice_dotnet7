...
namespace Northwind.Common.EntityModels.Test
{
    public class NorthwindEntityModelsTests
    {
        ...
        public void ConnectionIsTrue()
        {
            ...
            { // Arrange
                bool canConn = db.Database.CanConnect(); // Act
                Assert.True(canConn); // Assert
            }
        }
        ...
        public void EmployeeHasLastRefreshedIn10sWindow()
        {
            using (NorthwindContext db = new())
            {
                ...
                Assert.InRange(actual: employee1.LastRefreshed,
                    low: now.Subtract(TimeSpan.FromSeconds(5)), high: now.AddSeconds(5));
            }
        }
    }
}